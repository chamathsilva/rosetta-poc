...
**Start at `docs/EVALUATION-METHOD.md`** — it is the entry point for the evaluation and says which of the four layers answers which question, how evidence gets captured, and what the known gaps are.

This file is layer 2 of 4: the **raw cost ledger**. The others are `docs/EVALUATION-SESSIONS/*.md` (what actually happened in a chunk, in detail), `docs/EVALUATION-FINDINGS.md` (the cross-run synthesis — the analysis, not the record), and `agents/MEMORY.md` (the generalized rules).
...
| 2026-09-08/09 | Gated deploy — full `coding-flow` run: discovery, design, specs, plan, plan review, B1–B7, code review, validation, fixes | rosetta | not separately tracked; upper-bound only | **~1,853,000** across 12 completed subagent runs (2 further runs died at a rate limit and reported nothing) | **~1,853,000+** | 2 calendar days, including an overnight rate-limit wait — not 2 days of work | Per-agent breakdown, HITL gates with rejected options, and the 17-defect "which pass caught it" table: `docs/EVALUATION-SESSIONS/2026-09-08-gated-deploy.md`. Two figures are **cumulative across resumed runs** (architect 223,295; B3 151,366) — summing every reported figure would double-count. **~2.2× the cost of building the feature this deploys** (walking skeleton, ~830,000). Nothing provisioned; 31 of 84 acceptance criteria await a droplet. |
| 2026-09-10 | G4 gate — two rounds of human review on the provisioning runbook, both with fixes applied | rosetta | orchestrator-direct, no subagents; not reliably measurable (see note above) | 0 | n/a | ~2 hours across two review-and-fix cycles | Ten real, execution-blocking defects total (5 + 5), zero overlap with either the AI code review or the AI validation pass that had already cleared the same document. Also surfaced and fixed in this window: a merge that landed one commit behind its own fix (PR #9), an orchestrator direct-push to the protected default branch (self-caught, not reverted), and a false-negative merge verification caused by an unchecked failed `git fetch`. Full account: `docs/EVALUATION-SESSIONS/2026-09-08-gated-deploy.md` §§5c–5d. |
...
### Qualitative observations — gated deploy, 2026-09-08/09

- **The honest headline: an independent plan review caught a defect that would have shipped a privacy control that ran daily, reported success, and erased nothing.** The retention task's three statements were designed to run in one transaction; `bans` has no migration, and Postgres aborts the *entire* transaction on a reference to a missing relation. Unguarded, the very first scheduled run would have rolled back `messages.ip`'s erasure along with the `bans` statement that failed — silently, every day, forever, while the systemd unit reported success. Three authoring passes (the initial design, the tech-spec/plan authoring, and a plan revision) wrote past this before a reviewer caught it by reading the migration file's own comment rather than trusting the spec's description of it.
- **The process defect underneath the technical one is the more transferable finding.** The plan itself carried the instruction "stop-and-report if the `bans` question changes scope" — and the same authoring pass that wrote that escalation trigger also implemented around the gap it existed to catch, without the report ever firing. A written escalation condition is not evidence that escalation happened; nothing outside that single pass existed to check whether it did. Generalized in `agents/MEMORY.md`.
- **A second, smaller instance of the same "claim survives without re-verification" failure mode this project has now hit twice.** "Six existing upgrade tests" was stated in discovery, repeated in the architecture notes, and repeated again in the plan; the real count was seven. Each document trusted the prior one's count rather than re-running a one-line `grep`. Cheap to verify at every hop, verified at none until a reviewer did.
- **Cost accounting for this feature is itself an example of the discipline it is supposed to be measuring.** Discovery's subagent cost (129,705 tokens) was captured; the design, plan, and several implementation batches were not tracked with the same rigor in this session's own working state file. The row above is recorded as incomplete rather than backfilled with an estimate, per this file's own rule that a log containing only clean numbers cannot support a finding.
