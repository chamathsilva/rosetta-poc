...
**Still open**, and named honestly rather than closed optimistically: nothing is deployed, so 31 of 84 acceptance criteria await a droplet; the retention promise is *partial* until the moderation migration lands; no backups, no monitoring, no rate limiting, no real domain; the capacity ceiling is unmeasured. See `docs/TODO.md`.
