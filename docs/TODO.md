...
`docs/EVALUATION-SESSIONS/2026-09-08-ci-and-ai-review.md` and `2026-09-08-gated-deploy.md` are captured in full but **not yet synthesized** into the thematic findings. The session records report; `EVALUATION-FINDINGS.md` argues. As of 2026-09-10 the gated-deploy record alone carries 23 attributed defects and four themes ready to state: (1) execution and inspection find **disjoint** defect sets in this codebase, observed repeatedly; (2) a recurring defect shape — a claim true as written and false in fact, because the check and the claim look at different things — with six instances now; (3) a human-executed sequential document (the runbook) needed its own review method — simulating execution in order — because reading it, even carefully, and even mutating code to test it, did not surface any of ten real defects across two AI passes; (4) verification-of-verification has its own failure mode: a ref-diff is only as trustworthy as the fetch that populated the ref, and a merge can land behind its own just-pushed fix with no error anywhere in the chain. Protocol: `docs/EVALUATION-METHOD.md`.
