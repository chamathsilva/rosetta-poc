...
Status values — **all five that appear in this file**, declared because an earlier version of this line named only two while five were in use (code review, 2026-09-09):

| Status | Means |
|---|---|
| **PLANNED** | Decided, not built. |
| **SCAFFOLDED** | Built far enough to prove the toolchain, not to carry product behaviour. |
| **IMPLEMENTED** | Built **and** exercised — it has actually run. |
| **IMPLEMENTED, unexecuted (no droplet)** | Code or config exists and is reviewed, but has **never run**, because no host exists. Not the same as IMPLEMENTED, and the distinction is load-bearing: this project has twice shipped defects that only execution could find. |
| **ACTIVE, UNDECIDED** | Running in the project, but not recorded as a decision anywhere — found rather than chosen. |
...
| Reverse proxy & TLS | Caddy (automatic HTTPS certificates), gate + reverse-proxy config written (`deploy/Caddyfile`) | IMPLEMENTED, unexecuted (no droplet) |
| Process supervision | systemd — 4 units written (`deploy/systemd/`): `chat.service`, `chat-migrate.service`, `rosetta-chat-retention.{service,timer}` | IMPLEMENTED, unexecuted (no droplet) |
| Secrets management | systemd `EnvironmentFile` — **resolved**, split by scope, not a single file: `/etc/rosetta-chat/secrets.env` (host-scoped, `DATABASE_URL`/`JWT_SECRET`) + release-scoped `deploy.env` (`ALLOWED_ORIGIN`/`PORT`/`NODE_ENV`) | IMPLEMENTED, unexecuted (no droplet) |
...
| Code review automation | **`anthropics/claude-code-action@v1`**, the workflows generated by `/install-github-app` (merged to `develop` as PR #4) — `claude-code-review.yml` posts **inline** PR comments from a repository-grounded prompt (the generated `code-review` plugin config was removed — broken upstream, `claude-code-action#1290`); `claude.yml` answers `@claude`. Auth: `CLAUDE_CODE_OAUTH_TOKEN` repo secret, added 2026-09-08. **The vendor defaults were kept over hand-written alternatives [USER-DECIDED — 2026-09-08]**, so there is no draft-PR or fork-PR guard | IMPLEMENTED |
...
| CD (deploy to droplet) | GitHub Actions — `.github/workflows/deploy.yml`, `workflow_dispatch`-only. Rebuilds server and client on its own runner and ships a self-contained release over SSH; **by decision, nothing ever consumes CI's `dist/client` artifact** — a cross-run lookup would insert a mapping step between "the ref a human picked" and "what ships" (`plans/gated-deploy/architecture-notes.md` §3). **Written but never executed — no droplet exists (`docs/RUNBOOK-gated-deploy.md`, [HOST])** | IMPLEMENTED, unexecuted (no droplet) |
